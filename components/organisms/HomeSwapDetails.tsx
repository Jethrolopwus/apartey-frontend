...
const HomeSwapDetails: React.FC<HomeSwapDetailsProps> = ({ id }) => {
  ...
  const thumbnails =
    property.media?.uploads?.filter((u) => u.type === "image") || [];
          ...
          <div className="flex md:flex-col gap-2 md:ml-2">
            {thumbnails.map((thumb, i: number) => (
              <Image
                ...
                width={112}
                height={80}
                className="w-24 h-16 md:w-28 md:h-20 object-cover rounded-lg border border-gray-200"
              />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="mb-4">
            ...
            <ul className="list-disc list-inside text-gray-600 text-sm space-y-1">
              {amenities.length > 0 ? (
                amenities.map((a: string, i: number) => <li key={i}>{a}</li>)
              ) : (
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="bg-gray-50 rounded-lg border p-4 flex flex-col items-center text-center mb-4">
            ...
            <div className="text-xs text-gray-500 mb-2">
              {host.typeOfOffer || "Host"} • 4.8 (120 reviews)
            </div>
            <button className="bg-[#C85212] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-[#A64310] mb-2 w-full">
              Message Host
            </button>
            <button className="bg-white border border-[#C85212] text-[#C85212] px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-100 w-full">
              Request Swap
            </button>
          </div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-lg p-4 border flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-700">
                Great place, very clean and the host was super helpful. Would
          ...
          <div className="bg-gray-50 rounded-lg p-4 border flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-700">
                Perfect location and amenities. Responsive host. I hope to stay
                again soon!
              </div>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="flex gap-4 overflow-x-auto pb-2">
          {relatedProperties.length > 0
            ? relatedProperties.map((rel: Property, i: number) => (
                <div
                  key={rel._id || i}
                  className="min-w-[220px] bg-white border rounded-lg shadow-sm"
                >
                  <Image
                    src={rel.media?.coverPhoto || "/Estate2.png"}
                    alt="Related Home"
                    width={220}
                    height={128}
