...
const PropertyListings = () => {
  ...
  const handleSubmit = () => {
    ...
    // Validate enums
    if (!PropertyTypeEnum.includes(formData.propertyType || "")) {
      console.log("The property", PropertyTypeEnum);
    ...
    apiFormData.append("propertyDetails.price", String(formData.price || ""));
    apiFormData.append(
      "propertyDetails.negotiatedPrice",
      String(formData.isNegotiated || "")
    );
    apiFormData.append(
      "propertyDetails.totalFloors",
      String(formData.totalFloors || "")
    );
    apiFormData.append("propertyDetails.floor", String(formData.floor || ""));
    apiFormData.append(
      "propertyDetails.totalAreaSqM",
      String(formData.totalArea || "")
    );
    apiFormData.append(
      "propertyDetails.livingAreaSqM",
      String(formData.livingArea || "")
    );
    apiFormData.append(
      "propertyDetails.kitchenAreaSqM",
      String(formData.kitchenArea || "")
    );
    apiFormData.append(
    ...
    apiFormData.append(
      ...
      String(formData.bathrooms || "")
    );
    apiFormData.append(
      "propertyDetails.parkingSpots",
      String(formData.parkingSpots || "")
    );
    (formData.amenities || []).forEach((item) => {
      apiFormData.append("propertyDetails.amenities[]", String(item));
    });
    ...
    // Contact Info (dot notation)
    apiFormData.append(
      "contactInfo.typeOfOffer",
      String(formData.typeOfOffer || "")
    );
    apiFormData.append(
    ...
    apiFormData.append(
      "contactInfo.openForTourSchedule",
      String(formData.openForTour || "")
    );

    // Ad Promotion (dot notation, if present)
    if (formData.adPromotion) {
      if (
        "selectedTier" in formData.adPromotion &&
        formData.adPromotion.selectedTier
      )
        apiFormData.append(
          "adPromotion.selectedTier",
          String(formData.adPromotion.selectedTier)
        );
      if (
        "certifiedByFinder" in formData.adPromotion &&
        formData.adPromotion.certifiedByFinder
      )
        apiFormData.append(
          "adPromotion.certifiedByFinder",
          String(formData.adPromotion.certifiedByFinder)
        );
      if (
        "liftsToTopCount" in formData.adPromotion &&
        formData.adPromotion.liftsToTopCount
      )
        apiFormData.append(
          "adPromotion.liftsToTopCount",
          String(formData.adPromotion.liftsToTopCount)
        );
      if (
        "detailedAnalytics" in formData.adPromotion &&
        formData.adPromotion.detailedAnalytics
      )
        apiFormData.append(
          "adPromotion.detailedAnalytics",
          String(formData.adPromotion.detailedAnalytics)
        );
      if (
        "totalPrice" in formData.adPromotion &&
        formData.adPromotion.totalPrice
      )
        apiFormData.append(
          "adPromotion.totalPrice",
          String(formData.adPromotion.totalPrice)
        );
    }
