...
const ReviewsSection: React.FC<ReviewsSectionProps> = ({
  ...
  const handleSortChange = (option: SortOption) => {
    ...
    // Optionally sort reviews here based on selected sortOption
  };
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <h4 className="text-gray-800 font-medium text-lg">
                      {review.location.streetAddress}, {review.location.district},{" "}
                      {review.location.city}
                    </h4>
                    <div className="flex items-center gap-2 my-1">
                      <div className="flex">{renderStars(review.location.streetAddress)}</div>
                      <span className="text-gray-700">{review.overallRating.toString()}</span>
                    </div>
                    <p className="text-gray-500 text-sm">{review.detailedReview}</p>
                  </div>
                </article>
              ))
            ) : (
