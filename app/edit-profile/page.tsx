...
export default function EditProfilePage() {
  ...
  if (isError) return <div className="p-8 text-red-500">{(error as any)?.message || "Error loading profile."}</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      />
      {updateError && (
        <div className="text-red-500 text-center mt-4">{(updateError as any)?.message || "Error updating profile."}</div>
      )}
    </div>
  );
}
